...
//这个是用户自己的序列化实现,用于支持一些其他库里的类型
#ifdef XUEXUE_JSON_USER_SERIALIZE
    template <class T>
    static Value&& toValue(const std::shared_ptr<T>& obj, Value&& jv, rapidjson::MemoryPoolAllocator<>& allocator);
    template <class T>
    static ValueW&& toValue(const std::shared_ptr<T>& obj, ValueW&& jv, rapidjson::MemoryPoolAllocator<>& allocator);

    template <class T>
    static void getObj(const Value& value, std::shared_ptr<T>& obj);
    template <class T>
    static void getObj(const ValueW& value, std::shared_ptr<T>& obj);

    template <class T>
    static Value&& toValue(const T& obj, Value&& jv, rapidjson::MemoryPoolAllocator<>& allocator);
    template <class T>
    static ValueW&& toValue(const T& obj, ValueW&& jv, rapidjson::MemoryPoolAllocator<>& allocator);

    template <class T>
    static void getObj(const Value& value, T& obj);
    template <class T>
    static void getObj(const ValueW& value, T& obj);
...
#endif // XUEXUE_JSON_USER_SERIALIZE
